...
std::vector<std::pair<std::function<void()>, std::string>>& TestCase::getStackTest() {
    static std::vector<std::pair<std::function<void()>, std::string>> stack_test;
    return stack_test;
}
...
SubCase::SubCase(const std::string& name) : m_name_subcase(name), m_deep_subcase(getDeep()) {
...
std::vector<std::vector<std::string>>& SubCase::getStackSubCase() {
    static std::vector<std::vector<std::string>> stack_subcase(DEEP_SUBCASE,
                                                               std::vector<std::string>());
    return stack_subcase;
}

bool SubCase::tryInSubCase(int deep) {
    return static_cast<unsigned int>(getCountSubCase()[deep]) == getStackSubCase()[deep].size();
}
...
// cppcheck-suppress unusedFunction
void TestCase::saveTest(const std::function<void()>& func, const std::string& msg) {
...
inline void check(bool expr,
    ...
    std::cerr << "  CHECK(" << string_expr << ") at " << file << ":" << line << " failed!\n";
    if (msg.has_value()) {
        std::cerr << "      message: " << msg.value() << "\n";
    }
    if (SubCase::getDeep() != 0 && !SubCase::getInCase()[SubCase::getDeep() - 1].empty()) {
        for (int i = 0; i < 30; ++i) {
...
inline int run_test() {
    ...
    std::vector<std::pair<std::function<void()>, std::string>>& stack = TestCase::getStackTest();
    ...
    for (const auto& pair : TestCase::getStackTest()) {
        ...
        pair.first();
        while (static_cast<unsigned int>(SubCase::getCountSubCase()[SubCase::getDeep()]) !=
    ...
    std::cerr << "\n===== Tests passed: " << count_succes_test << "/" << count_tests << " =====\n";
...
#define CHECK(expr) \
    ::mytest::check(static_cast<bool>(expr), #expr, __FILE__, __LINE__, ::std::nullopt)
...
#define MYTEST_INTERNAL_NEW_NAME1(name_x, name_y) MYTEST_INTERNAL_NEW_NAMED(name_x, name_y)

#define MYTEST_INTERNAL_NEW_VAR MYTEST_INTERNAL_NEW_NAME1(mytest_var_, __LINE__)

#define MYTEST_INTERNAL_NEW_FUNC MYTEST_INTERNAL_NEW_NAME1(mytest_func_, __LINE__)

#define MYTEST_INTERNAL_NEW_SUBCASE MYTEST_INTERNAL_NEW_NAME1(mytest_subcase_, __LINE__)

#define TEST_CASE_REGISTER(func, msg) \
    static int MYTEST_INTERNAL_NEW_VAR = ((::mytest::TestCase::saveTest(func, msg)), 0)
...
#define SUBCASE(msg)                                                                 \
    if (::mytest::SubCase MYTEST_INTERNAL_NEW_SUBCASE(msg);                          \
        ::mytest::SubCase::getEntered()[::mytest::SubCase::getEnteredDeep()] == 0 && \
