...
matrix_error::matrix_error(const std::string& comment) : std::runtime_error(comment) {};

out_of_bounds_error::out_of_bounds_error() : matrix_error("Requested element is out of bounds") {};

dimension_mismatch_error::dimension_mismatch_error(int lhs, int rhs)
    : matrix_error("Dimension mismatch: lhs=" + std::to_string(lhs) +
                   ", rhs=" + std::to_string(rhs)) {};

Matrix::Matrix()
    : my_matrix(std::vector<std::vector<int>>(0, std::vector<int>(0))), my_rows(0), my_cols(0) {
}
    ...
    for (int i = 0; i < lhs.get_rows(); ++i) {
        for (int j = 0; j < rhs.get_cols(); ++j) {
            ...
            for (int r = 0; r < lhs.get_cols(); ++r) {
                buff_arithmetics +=
                    static_cast<int64_t>(lhs.at(i, r)) * static_cast<int64_t>(rhs.at(r, j));
            }
