...
bank::user& hello_user(boost::asio::ip::tcp::iostream& client, bank::ledger& m_ledger) {
...
void balance(boost::asio::ip::tcp::iostream& client, const bank::user& client_user) {
    client << client_user.balance_xts() << "\n";
}

void transfer(boost::asio::ip::tcp::iostream& client,
    ...
    comment.erase(0, 1);
    try {
        client_user.transfer(m_ledger.get_or_create_user(counterparty), amount, comment);
        client << "OK\n";
...
bank::user_transactions_iterator transaction(boost::asio::ip::tcp::iostream& client,
    ...
    bank::user_transactions_iterator client_iterator =
        client_user.snapshot_transactions([&](auto& m_transaction, int m_balance) {
            ...
            for (; i < m_transaction.size(); ++i) {
                if (m_transaction[i].counterparty == nullptr) {
                    ...
                    client << m_transaction[i].counterparty->name() << "\t"
                           << m_transaction[i].balance_delta_xts << "\t" << m_transaction[i].comment
                           << "\n";
                }
            }
...
void monitor(boost::asio::ip::tcp::iostream& client, const bank::user& client_user) {
    bank::user_transactions_iterator client_iterator = transaction(client, client_user);
    while (true) {
        const bank::transaction client_next_transaction = client_iterator.wait_next_transaction();
