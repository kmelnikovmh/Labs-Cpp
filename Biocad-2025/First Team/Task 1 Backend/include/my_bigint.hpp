    ...
    bigint operator*(const bigint& other) const {
        ...
        for (size_t i = 0; i < digits.size(); ++i) {
            ...
            for (size_t j = 0; j < other.digits.size(); ++j) {
                long long product = static_cast<long long>(digits[i]) * other.digits[j] +
                                    result.digits[i + j] + carry;
                result.digits[i + j] = static_cast<int>(product % BASE);
                carry = static_cast<int>(product / BASE);
            }
            if (carry > 0) {
                if (i + other.digits.size() >= result.digits.size()) {
                    result.digits.push_back(static_cast<int>(carry));
                } else {
                    result.digits[i + other.digits.size()] += static_cast<int>(carry);
                }
            }
        }
